...
class Solver(object):
    def __init__(self, data, model, optimizer, args):
        ...
        if args.bandmask:
            augments.append(
                augment.BandMask(args.bandmask, sample_rate=args.sample_rate)
            )
        if args.shift:
            augments.append(augment.Shift(args.shift, args.shift_same))
        if args.revecho:
            augments.append(augment.RevEcho(args.revecho))
        ...
        if self.checkpoint:
            ...
            logger.debug(
                "Checkpoint will be saved to %s", self.checkpoint_file.resolve()
            )
        self.history_file = args.history_file
        self.predict_both = args.predict_both
        ...
        self.mrstftloss = MultiResolutionSTFTLoss(
            factor_sc=args.stft_sc_factor, factor_mag=args.stft_mag_factor
        ).to(self.device)
        self._reset()
    ...
    def _reset(self):
        ...
        if self.args.load_pretrained:
            logger.info("Loading pre-trained model %s", self.args.load_pretrained)
            # model = getattr(pretrained, self.args.load_pretrained)()
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            if (
                (epoch + 1) % self.eval_every == 0 or epoch == self.epochs - 1
            ) and self.tt_loader:
            ...
            if distrib.rank == 0:
                ...
                if self.checkpoint:
                    self._serialize()
                    logger.debug(
                        "Checkpoint saved to %s", self.checkpoint_file.resolve()
                    )
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            if self.predict_both:
                target = torch.cat([noisy - clean, clean], axis=1)
            # apply a loss function after each layer
            with torch.autograd.set_detect_anomaly(True):
                if self.args.loss == "l1":
                    loss = F.l1_loss(target, estimate)
                elif self.args.loss == "l2":
                    loss = F.mse_loss(target, estimate)
                elif self.args.loss == "huber":
                    loss = F.smooth_l1_loss(target, estimate)
                else:
                    raise ValueError(f"Invalid loss {self.args.loss}")
                # MultiResolution STFT loss
                if self.args.stft_loss:
                    sc_loss, mag_loss = self.mrstftloss(
                        estimate.squeeze(1), target.squeeze(1)
                    )
